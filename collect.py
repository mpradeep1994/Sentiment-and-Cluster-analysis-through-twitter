...
#find the unique users by avoiding repetition 
...
#finding the friends of the each user
...
#build graph by drawing edges for users who follow mutually two teams 
...
"""draw edges for the users and the team"""
...
"""
store back the result to pickle file
"""
...
def main():
    ...
    count=0
    """open the file and  find the unique users count
    """
    ...
    teams = list(names.keys())
    #find the mutual users following the each teams
    for i in range(len(teams)):
        for j in range(i+1,len(teams)):
                a =list(set(names[teams[i]] ) & set(names[teams[j]]))
                print ("Mutual number of followers for the team",teams[i],teams[j],"---->",len(a))           
    #draw the graph by building the edges betwween the nodes 
    graph=draw_graph(names) 
    g=read_graph(graph,names)
    print('graph has %d nodes and %d edges' %(graph.order(), graph.number_of_edges()))
    #pass the graph to girvan newman to identify communities
    result=girvan_newman(g,10, 20)
    name=1
    TEMP={}
    #add the node details and user count details to pickle file for summerize
